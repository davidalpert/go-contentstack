...
func NewEnvironmentUpdateOptions(s printers.IOStreams) *EnvironmentUpdateOptions {
	return &EnvironmentUpdateOptions{
...
func NewCmdEnvironmentUpdate(s printers.IOStreams) *cobra.Command {
	o := NewEnvironmentUpdateOptions(s)
	var cmd = &cobra.Command{
		Use:     "update <name>",
		Short:   "update the urls for a publish environment with the given name",
		Aliases: []string{"u", "up"},
...
// Run the command
func (o *EnvironmentUpdateOptions) Run() error {
	if o.FormatCategory() == "table" || o.FormatCategory() == "csv" {
		o.WithDefaultOutput("json")
	}
	e, err := o.Client.GetOnePublishingEnvironment(o.Name)
	if err != nil {
		return err
	}

	e.Urls = []schema.LocaleURLPair{}
	for l, u := range o.URLsByLocale {
		e.Urls = append(e.Urls, schema.LocaleURLPair{Locale: l, Url: u})
	}
	r, err := o.Client.UpdatePublishingEnvironment(e)
	if err != nil {
		return err
	}
